    ...
    ***************************************************************************/
	...
	private void printTree(Node node) {
    	if(node == null) {
    		...
    		if(node.key.compareTo(root.key) == 0) System.out.println("Tree: ");
        	printTree(node.left);
        	System.out.printf("Key: %s\t  Color: %s\t%s\n",node.key, ((node.color == RED)? "Red" : "Black"), ((node.key.compareTo(root.key) == 0)? " Root" : ""));
        	//System.out.printf("Key: %s\t   Value: o\tColor: %s\t%s\n",node.key, node.val, ((node.color == RED)? "Black" : "Red"), ((node.key.compareTo(root.key) == 0)? "Root" : ""));
